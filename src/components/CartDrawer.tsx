...
export const FloatingCartButton: React.FC = () => {
  ...
  return (
    <>
      ...
      {lastAddedProduct && (
        <div className="fixed bottom-22 right-6 z-50 animate-in slide-in-from-bottom-3 duration-300 bg-slate-900 text-white px-4 py-2.5 rounded-xl shadow-2xl border border-brand-teal/40 flex items-center space-x-3 text-xs">
      ...
      <button
        onClick={openCart}
        className="fixed bottom-6 right-6 z-40 flex items-center space-x-2.5 bg-brand-teal hover:bg-[#20968E] text-white px-4 py-3 rounded-full shadow-medical-hover border-2 border-white transition-all transform hover:scale-105 group cursor-pointer"
        aria-label="View quotation cart"
      >
        <div className="relative">
          <ShoppingCart className="w-5 h-5 text-white" />
          {cartItems.length > 0 && (
            <span className="absolute -top-2.5 -right-2.5 bg-[#0D3666] text-white text-[10px] font-mono font-extrabold w-5 h-5 rounded-full flex items-center justify-center border-2 border-white shadow-xs">
              {cartItems.length}
            </span>
          )}
        </div>
        <span className="font-display font-bold text-xs tracking-wide hidden sm:inline">
          Wholesale Cart
        </span>
        {cartItems.length > 0 && (
